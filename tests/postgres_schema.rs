...
fn explores_schemas_relations_and_details() {
    let url = env::var("DATABASE_URL").expect("DATABASE_URL must point to the disposable test database");
    ...
    assert!(schemas.iter().any(|schema| schema.name == "ide_schema_test"));
    let relations = list_relations(&mut client, "ide_schema_test").expect("list relations");
    assert_eq!(relations.iter().find(|relation| relation.name == "customers").map(|relation| relation.relation_type.as_str()), Some("TABLE"));
    let details = describe_relation(&mut client, "ide_schema_test", "customers").expect("describe relation");
