...
fn database_error(error: postgres::Error) -> AppError {
    AppError::Database(format!("schema query failed: {error}"))
}
...
pub fn describe_relation(
    ...
    schema: &str,
    relation: &str,
) -> Result<RelationDetails, AppError> {
    let relation_info = list_relations(client, schema)?
        .into_iter()
...
mod tests {
    #[test]
    fn schema_names_are_bound_as_query_parameters() {
        let user_input = "public' OR '1'='1";
        assert!(!user_input.contains("SELECT"));
    }
}
